...
SETUPTOOLS=`cat versions.cfg | grep "setuptools\s*\=\s*" | sed 's/ *$//g' | sed 's/=$//g' | sed 's/\s*=\s*/==/g'`
ZCBUILDOUT=`cat versions.cfg | grep "zc\.buildout\s*=\s*" | sed 's/\s*=\s*/==/g'`
...
curl -o "/tmp/virtualenv.py" -k "https://raw.githubusercontent.com/pypa/virtualenv/1.10.X/virtualenv.py"
